...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  Node * current_node = this->getHeadPtr();
  int size = this->size();
  ...
  while (current_node) {
    if (current_node->data < newData && current_node->next && current_node->next->data > newData) {
      ...
      this->size_++;
      break;
    } else if (current_node->data > newData) {
      this->pushFront(newData);
      break;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // and an int.)
  Node * current_left = left.head_;
  Node * current_right = right.head_;

  while (current_left || current_right) {
    if (current_right && current_left && current_left->data < current_right->data) {
      merged.pushBack(current_left->data);
      current_left = current_left->next;
    } else if (current_right && current_left && current_left->data > current_right->data) {
      merged.pushBack(current_right->data);
