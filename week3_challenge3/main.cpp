...
public:
  ...
  // value.)
  bool has_cycle[MAX_NODES]; // [false, false, false, ...]
...
void DisjointSets::dsunion(int i, int j) { // Args (0, 1)
  bool i_had_cycle = query_cycle(i); // false
  bool j_had_cycle = query_cycle(j); // false
  int root_i = find_leader(i); // 0
  int root_j = find_leader(j); // 1
  if (root_i != root_j) {
