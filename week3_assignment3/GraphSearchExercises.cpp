...
int GridGraph::countEdges() const {
  ...
  // std::cout << "Adjacency Map" << adjacencyMap << std::endl;
  // =======================================================================
  // TODO: Your code here!
  // =======================================================================
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  // for (IntPair k : originalNeighbors) {

    // This long version is for myself to know what should "auto" be
    for (std::unordered_map<IntPair, GridGraph::NeighborSet>::iterator it = adjacencyMap.begin(); it != adjacencyMap.end(); ++it) {
      GridGraph::NeighborSet it_neighbors = it->second; // unordered_set<IntPair>

      for (auto neighbor = it_neighbors.begin(); neighbor != it_neighbors.end(); ++neighbor) { // neighbor = IntPair
        if ((neighbor->first == p1.first) && (neighbor->second == p1.second)) {
          it->second.erase(p1);
        }
      }
    }
...
std::list<IntPair> graphBFS(const IntPair& start, const IntPair& goal, const GridGraph& graph) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    // std::unordered_map<IntPair, GridGraph::NeighborSet> adj = graph.adjacencyMap;
    // =====================================================================
...
std::list<PuzzleState> puzzleBFS(const PuzzleState& start, const PuzzleState& goal) {
  ...
  while (!exploreQ.empty() && !foundGoal && !tooManySteps) {
    ...
    // auto neighbors = start.getAdjacentStates(); // Change this! This line is totally wrong.
    ...
    for (auto neighbor : neighbors) {
      ...
      if (!neighborWasAlreadyVisited) {
        ...
        // for (auto state : curState.getAdjacentStates()) {
        //   if (!visitedSet.count(state)) {
        //     exploreQ.push(state);
        //   }
        // }
