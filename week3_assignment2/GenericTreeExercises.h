...
static void treeFactory(GenericTree<int>& tree) {
  ...
  using TreeNode = GenericTree<int>::TreeNode;

  TreeNode* root = tree.getRootPtr();

  tree.deleteSubtree(root);
  tree.createRoot(4);
  root = tree.getRootPtr();
  TreeNode* node8 = root->addChild(8);
  TreeNode* node16 = node8->addChild(16);
  TreeNode* node23 = node8->addChild(23);
  TreeNode* node42 = node16->addChild(42);
  TreeNode* node15 = root->addChild(15);
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  to_process.push_back(rootNodePtr);
  int process_pointer = 0;

  while (process_pointer < (int)to_process.size()) {
    std::vector<TreeNode*> children_pointers = to_process[process_pointer]->childrenPtrs;
    for (int i = 0; i < (int)children_pointers.size(); i++) {
      to_process.push_back(children_pointers[i]);
    }

    results.push_back(to_process[process_pointer]->data);
    process_pointer++;
  }
