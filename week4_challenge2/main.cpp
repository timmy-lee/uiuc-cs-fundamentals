...
void downHeap(Node *n) {
  if (n == NULL || (n->left == NULL && n->right == NULL)) {
    ...
    if (n->value > n->left->value) {
      Node * roots_left = n->left;
      Node * lefts_right = n->left->right;
      Node * lefts_left = n->left->left;
      n->left = lefts_left;
      n->right = lefts_right;
      roots_left->left = n;
      downHeap(n);
    ...
    if (n->value > n->right->value) {
      Node * roots_right = n->right;
      Node * rights_right = n->right->right;
      Node * rights_left = n->right->left;
      n->left = rights_left;
      n->right = rights_right;
      roots_right->right = n;
      downHeap(n);
    }
  } else {
    int root_value = n->value;
    int left_value = n->left->value;
    int right_value = n->right->value;

    if (left_value < right_value && root_value > left_value) {
      Node * roots_right = n->right;
      Node * roots_left = n->left;
      Node * lefts_left = n->left->left;
      Node * lefts_right = n->left->right;
      ...
      roots_left->left = n;
      roots_left->right = roots_right;
      n->left = lefts_left;
      n->right = lefts_right;

      downHeap(n);
    } else if (left_value >= right_value && root_value > right_value) {
      Node * roots_right = n->right;
      Node * roots_left = n->left;
      Node * lefts_left = n->left->left;
      Node * lefts_right = n->left->right;
...
int main() {
  ...
  bool equals = y->right == NULL;
  cout << "Is this equal? " << equals << endl;

  Node *d = new Node(100);
  Node *tracking = new Node(12);
  d->right = tracking;
  d->right->right = new Node(15);
  d->right->right->right = new Node(19);
  downHeap(d);
  printTree(tracking);

  Node *e = new Node(100);
  Node *trackingE = new Node(12);
  e->left = trackingE;
  e->left->left = new Node(15);
  e->left->left->left = new Node(19);
  downHeap(e);
  printTree(trackingE);
