...
void DisjointSets::bfs(int i, int n, int m, int edges[][2]) {
  ...
  for (int d = 1; d < m; d++) {
    ...
    int rooti = find(i); // find(3) => return -1
    ...
    for (int j = 0; j < m; j++) {
      ...
      int root0 = find(edges[j][0]); // {0, 1} => find(0) => return 0 when j = 0;
      int root1 = find(edges[j][1]); // {0, 1} => find(1) => return 1 when j = 0;

      cout << "============What I have before conditionals============" << endl;
      cout << "d:           " << d << endl;
      cout << "j:           " << j << endl;
      cout << "edges[j][0]: " << edges[j][0] << endl;
      cout << "edges[j][1]: " << edges[j][1] << endl;
      cout << "root0:       " << root0 << endl;
      ...
      if (rooti == root0 && rooti != root1) {
        cout << "Entered first condition" << endl;
        ...
        cout << "Entered second condition" << endl;
        if (f == -1)
          f = edges[j][0];
        else
          dsunion(f,edges[j][0]);
